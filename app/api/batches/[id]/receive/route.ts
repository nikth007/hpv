...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!links.length) return NextResponse.json({ error: 'Batch not found or has no samples' }, { status: 404 });
    ...
    for (const link of links) {
      ...
      if (missing.has(sampleId)) {
        ...
        await query(`UPDATE samples SET condition_status = 'MISSING', updated_at = now() WHERE id = $1`, [sampleId]);
        ...
        await query(`UPDATE samples SET status = 'RECEIVED_AT_HUB', condition_status = 'DAMAGED', updated_at = now() WHERE id = $1`, [sampleId]);
      } else {
        await query(
          `UPDATE dispatch_batch_samples SET receive_status = 'RECEIVED' WHERE dispatch_batch_id = $1 AND sample_id = $2`,
          [id, sampleId]
        );
        await query(`UPDATE samples SET status = 'RECEIVED_AT_HUB', condition_status = 'OK', updated_at = now() WHERE id = $1`, [sampleId]);
      }
    }
    ...
    await outboxEvent('BATCH_RECEIVED', 'dispatch_batch', id, batchRows[0]);
    return NextResponse.json({ batch: batchRows[0] });
  } catch (error: any) {
